...
export default async function CasesInboxPage() {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-2xl font-semibold text-stone-900">Case inbox</h1>
      ...
      <div className="overflow-x-auto rounded-lg border border-stone-200 bg-white shadow-sm">
            ...
            {rows.map(({ case: c, reservation: r }) => (
              <tr key={c.id} className="border-b border-stone-100 hover:bg-stone-50/80">
                ...
                <td className="px-3 py-2 text-xs">
                  ...
                  <div className="text-stone-500">{r.externalSource}</div>
                </td>
        ...
        {rows.length === 0 ? (
          <p className="p-6 text-center text-stone-500">No cases yet.</p>
        ) : null}
      </div>
    </div>
  );
