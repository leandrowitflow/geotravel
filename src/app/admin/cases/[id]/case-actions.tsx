...
export function CaseActions({ caseId }: { caseId: string }) {
  ...
  async function run(action: string) {
    ...
    try {
      ...
      if (!res.ok) {
        setNotice({ tone: "err", text: j.error ?? "Action failed", hint: typeof j.hint === "string" ? j.hint : undefined });
        return;
      }
      setNotice({
        tone: "ok",
  ...
  return (
    <div className="space-y-2">
      {notice && (
        <p className={notice.tone === "ok" ? "text-sm text-emerald-700 dark:text-emerald-300" : "text-sm text-red-700 dark:text-red-300"} role="status">
          {notice.text}
          {notice.hint && <span className="mt-1 block text-xs text-stone-600 dark:text-stone-400">{notice.hint}</span>}
        </p>
      )}
      <div className="flex flex-wrap gap-2">
        <button type="button" disabled={pending !== null} onClick={() => run("resend")} className="rounded-md bg-teal-800 px-3 py-1.5 text-sm text-white hover:bg-teal-900 disabled:opacity-50">
          {pending === "resend" ? "…" : "Resend reminder"}
        </button>
        <button type="button" disabled={pending !== null} onClick={() => run("force_sms")} className="rounded-md border border-stone-300 bg-white px-3 py-1.5 text-sm hover:bg-stone-50 disabled:opacity-50 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100 dark:hover:bg-stone-700">
        ...
        <button type="button" disabled={pending !== null} onClick={() => run("close_case")} className="rounded-md border border-stone-300 bg-white px-3 py-1.5 text-sm hover:bg-stone-50 disabled:opacity-50 dark:border-stone-600 dark:bg-stone-800 dark:text-stone-100 dark:hover:bg-stone-700">
